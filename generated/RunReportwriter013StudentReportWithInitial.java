...
public class RunReportwriter013StudentReportWithInitial extends CobolProgram {
    ...
    private CobolString input_rec = new CobolString(30); // Group: INPUT-REC
    ...
    private ReportWriter _rpt_student_report2 = new ReportWriter("STUDENT-REPORT2");
    ...
    private void a000_mainline() {
        ...
        if (input_file.read(input_rec) == FileStatus.AT_END) {
            ...
            try { num_courses = Integer.parseInt(CobolString.refMod(input_rec, 29, 2).toString().trim()); } catch (NumberFormatException _nfe) {}
        }
    ...
    public void run() {
        ...
        try {
            CobolProgram.registerCallAlias("prog", "RunReportwriter013StudentReportWithInitial");
            CobolProgram.registerCallAlias("prog", "RunReportwriter013StudentReportWithInitial");
            CobolDisplay.setDecimalPointIsComma(false);
            CobolDisplay.setDpcInDataMode("DEFAULT");
            CobolIntrinsics.setLocationEnabled(false);
            // IS INITIAL: re-initialize WORKING-STORAGE to VALUE clauses on each entry
            are_there_more_records.set("YES");
            a000_mainline();
