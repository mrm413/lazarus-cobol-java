package com.lazarus.cobol.generated;

import com.lazarus.cobol.CobolDisplay;
import com.lazarus.cobol.CobolFile;
import com.lazarus.cobol.CobolIntrinsics;
import com.lazarus.cobol.CobolProgram;
import com.lazarus.cobol.CobolString;
import com.lazarus.cobol.FileStatus;
import java.math.BigDecimal;
import java.math.RoundingMode;

public class RunFile032ReadOnOptionalMissingRelativeSequential extends CobolProgram {
    public static final String COBOL_PROGRAM_ID = "prog";
    private int sort_return = 0;
    private int json_code = 0;
    private int xml_code = 0;
    private int cob_crt_status = 0;
    private int length_an = 0;
    private CobolString when_compiled = new CobolString(21);
    private CobolString debug_item = new CobolString(256);
    private CobolString debug_line = new CobolString(6);
    private CobolString debug_name = new CobolString(30);
    private CobolString debug_sub_1 = new CobolString(4);
    private CobolString debug_sub_2 = new CobolString(4);
    private CobolString debug_sub_3 = new CobolString(4);
    private CobolString debug_contents = new CobolString(256);
    private CobolString stdin = new CobolString(256);
    private CobolString stdout = new CobolString(256);
    private CobolString stderr = new CobolString(256);
    private boolean _exitSection = false;
    // WORKING-STORAGE SECTION
    private CobolString wsfs = new CobolString(2);
    private CobolString wsinrec = new CobolString(80);

    // FILE SECTION — INFILE
    private CobolString inrec = new CobolString(80);

    // FILE SECTION — INFILES
    private CobolString inrecs = new CobolString(80);

    
    // VMG: flat-memory layout (Phase 1.5b — declarations only, not yet wired into MOVE/DISPLAY)
    private final com.lazarus.cobol.vmg.MemoryGrid _vmg_grid = new com.lazarus.cobol.vmg.MemoryGrid(82);
    @SuppressWarnings("unused") private final com.lazarus.cobol.vmg.FieldDescriptor _vmgfd_wsfs = com.lazarus.cobol.vmg.FieldDescriptor.builder().grid(_vmg_grid).at(0, 2).storage(com.lazarus.cobol.vmg.FieldDescriptor.Storage.ALPHANUMERIC).name("WSFS").build();
    @SuppressWarnings("unused") private final com.lazarus.cobol.vmg.FieldDescriptor _vmgfd_wsinrec = com.lazarus.cobol.vmg.FieldDescriptor.builder().grid(_vmg_grid).at(2, 80).storage(com.lazarus.cobol.vmg.FieldDescriptor.Storage.ALPHANUMERIC).name("WSINREC").build();

    // FILE DESCRIPTORS
    private CobolFile infile = new CobolFile("missing.txt", "RELATIVE", "SEQUENTIAL", "INFILE", true);
    private CobolFile infiles = new CobolFile("missings.txt", "SEQUENTIAL", "SEQUENTIAL", "INFILES", true);


    private void main_procedure() {
        infile.open("INPUT");
        wsfs.set(infile.getFileStatus());
        CobolDisplay.displayNoAdvancing("R: OPEN INPUT on missing optional file = " + String.valueOf(wsfs) + " ");
        if (CobolString.cobolEquals(String.valueOf(wsfs), String.valueOf("05"))) {
            CobolDisplay.display("OK");
        } else {
            CobolDisplay.display("Bad");
        }
        infiles.open("INPUT");
        wsfs.set(infiles.getFileStatus());
        CobolDisplay.displayNoAdvancing("S: OPEN INPUT on missing optional file = " + String.valueOf(wsfs) + " ");
        if (CobolString.cobolEquals(String.valueOf(wsfs), String.valueOf("05"))) {
            CobolDisplay.display("OK");
        } else {
            CobolDisplay.display("Bad");
        }
        infile.read(wsinrec);
        wsfs.set(infile.getFileStatus());
        CobolDisplay.displayNoAdvancing("R: 1st READ on missing optional file = " + String.valueOf(wsfs) + " ");
        if (CobolString.cobolEquals(String.valueOf(wsfs), String.valueOf("10"))) {
            CobolDisplay.display("OK");
        } else {
            CobolDisplay.display("Bad");
        }
        infiles.read(wsinrec);
        wsfs.set(infiles.getFileStatus());
        CobolDisplay.displayNoAdvancing("S: 1st READ on missing optional file = " + String.valueOf(wsfs) + " ");
        if (CobolString.cobolEquals(String.valueOf(wsfs), String.valueOf("10"))) {
            CobolDisplay.display("OK");
        } else {
            CobolDisplay.display("Bad");
        }
        infile.read(wsinrec);
        wsfs.set(infile.getFileStatus());
        CobolDisplay.displayNoAdvancing("R: 2nd READ on missing optional file = " + String.valueOf(wsfs) + " ");
        if (CobolString.cobolEquals(String.valueOf(wsfs), String.valueOf("46"))) {
            CobolDisplay.display("OK");
        } else {
            CobolDisplay.display("Bad");
        }
        infiles.read(wsinrec);
        wsfs.set(infiles.getFileStatus());
        CobolDisplay.displayNoAdvancing("S: 2nd READ on missing optional file = " + String.valueOf(wsfs) + " ");
        if (CobolString.cobolEquals(String.valueOf(wsfs), String.valueOf("46"))) {
            CobolDisplay.display("OK");
        } else {
            CobolDisplay.display("Bad");
        }
        infile.read(inrec);
        wsfs.set(infile.getFileStatus());
        CobolDisplay.displayNoAdvancing("R: 3rd READ on missing optional file = " + String.valueOf(wsfs) + " ");
        if (CobolString.cobolEquals(String.valueOf(wsfs), String.valueOf("46"))) {
            CobolDisplay.display("OK");
        } else {
            CobolDisplay.display("Bad");
        }
        infiles.read(inrecs);
        wsfs.set(infiles.getFileStatus());
        CobolDisplay.displayNoAdvancing("S: 3rd READ on missing optional file = " + String.valueOf(wsfs) + " ");
        if (CobolString.cobolEquals(String.valueOf(wsfs), String.valueOf("46"))) {
            CobolDisplay.display("OK");
        } else {
            CobolDisplay.display("Bad");
        }
        infile.close();
        wsfs.set(infile.getFileStatus());
        infiles.close();
        wsfs.set(infiles.getFileStatus());
        CobolIntrinsics.programExit(CobolProgram.return_code);
    }

    @Override
    public void run() {
        CobolIntrinsics.setModuleInfo("prog", "", "prog.cob");
        CobolProgram.pushActive("prog");
        try {
            CobolProgram.registerCallAlias("prog", "RunFile032ReadOnOptionalMissingRelativeSequential");
            CobolProgram.registerCallAlias("prog", "RunFile032ReadOnOptionalMissingRelativeSequential");
            CobolDisplay.setDecimalPointIsComma(false);
            CobolDisplay.setDpcInDataMode("DEFAULT");
            CobolIntrinsics.setLocationEnabled(false);
            if (infile != null && wsfs != null) infile.setStatusField(wsfs);
            if (infiles != null && wsfs != null) infiles.setStatusField(wsfs);
            main_procedure();
        } finally { CobolProgram.popActive(); }
    }

    public static void main(String[] args) {
        if (args.length > 0 && "--cobol-child".equals(args[0])) {
            CobolProgram.markIsCobolChild(true);
            String[] _rest = new String[args.length - 1];
            System.arraycopy(args, 1, _rest, 0, _rest.length);
            args = _rest;
        }
        CobolIntrinsics.setCommandLineArgs(args);
        new RunFile032ReadOnOptionalMissingRelativeSequential().run();
    }
}
