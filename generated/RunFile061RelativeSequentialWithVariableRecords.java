...
public class RunFile061RelativeSequentialWithVariableRecords extends CobolProgram {
    ...
    // FILE DESCRIPTORS
    private CobolFile f = new CobolFile("ORGANIZATION", "RELATIVE", "SEQUENTIAL", "F", 10, 20);

    private CobolString organization = new CobolString(256, "ORGANIZATION"); // fallback (implicit ASSIGN — initialized to name)

    private void para_main() {
        f.setFileName(String.valueOf(organization).trim());
        f.open("OUTPUT");
    ...
    public void run() {
        ...
        try {
            CobolProgram.registerCallAlias("prog", "RunFile061RelativeSequentialWithVariableRecords");
            CobolProgram.registerCallAlias("prog", "RunFile061RelativeSequentialWithVariableRecords");
    ...
    public static void main(String[] args) {
        ...
        new RunFile061RelativeSequentialWithVariableRecords().run();
    }
}
