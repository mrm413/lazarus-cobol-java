...
public class RunReportwriter009SampleChargeReport3 extends CobolProgram {
    ...
    private CobolString tr_customer = new CobolString(21); // Group: TR-CUSTOMER
    ...
    private void _000_initiate() {
        ...
        if (transaction_data.read(transaction_record) == FileStatus.AT_END) {
            ...
            tr_customer.set(CobolString.refMod(transaction_record, 1, 21));
        }
    ...
    private void _100_process_transaction_data() {
        ...
        _rpt_customer_report.generate("CUSTOMER-REPORT");
        if (transaction_data.read(transaction_record) == FileStatus.AT_END) {
            end_of_file_switch.set(String.valueOf("Y"));
        } else {
            try { tr_customer_number = Integer.parseInt(CobolString.refMod(transaction_record, 1, 4).toString().trim()); } catch (NumberFormatException _nfe) {}
    ...
    public void run() {
        ...
        try {
            CobolProgram.registerCallAlias("prog", "RunReportwriter009SampleChargeReport3");
            CobolProgram.registerCallAlias("prog", "RunReportwriter009SampleChargeReport3");
