...
public class RunMisc085SortTableWithDefaultCollatingSequence extends CobolProgram {
    ...
    private CobolString g = z; // Group REDEFINES: G
    ...
    private void para_main() {
        { /* SORT TABLE TBL (tbl) — in-memory */
            ...
            CobolTableSort.sort(10, _keys, _allArrays);
        ...
        if (!CobolString.cobolEquals(String.valueOf(g), String.valueOf("12345abcde"))) {
            { /* Build group G from children before group access */
                String _gbuf = g.get();
                StringBuilder _sb = new StringBuilder(10);
                for (int _bi = 0; _bi < 10; _bi++) {
    ...
    public void run() {
        ...
        try {
            CobolProgram.registerCallAlias("prog", "RunMisc085SortTableWithDefaultCollatingSequence");
            CobolProgram.registerCallAlias("prog", "RunMisc085SortTableWithDefaultCollatingSequence");
            CobolDisplay.setDecimalPointIsComma(false);
            CobolDisplay.setDpcInDataMode("DEFAULT");
            CobolIntrinsics.setLocationEnabled(false);
            // Initialize REDEFINES OCCURS children from Z
            {
                String _redef_g = z.get();
                for (int _i = 0; _i < 10; _i++) {
                    int _off = _i * 1 + 0;
                    if (x[_i] == null) x[_i] = new CobolString(1);
                    if (_off + 1 <= _redef_g.length()) x[_i].set(_redef_g.substring(_off, _off + 1));
                }
            }
            para_main();
    ...
    public static void main(String[] args) {
        ...
        new RunMisc085SortTableWithDefaultCollatingSequence().run();
    }
}
