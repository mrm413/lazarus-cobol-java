...
public class CicsRegion {
    ...
    private final Map<String, CicsVsamFile> vsamFiles = new ConcurrentHashMap<>();
    private final Map<String, CicsMap> maps = new ConcurrentHashMap<>();

    // ENQ/DEQ resource locks
    private final Map<String, Thread> enqLocks = new ConcurrentHashMap<>();
    ...
    public void registerVsamFile(String name, CicsVsamFile file) {
        vsamFiles.put(name.toUpperCase().trim(), file);
    }

    /** Get a VSAM file by dataset name */
    public CicsVsamFile getVsamFile(String name) {
        return vsamFiles.get(name.toUpperCase().trim());
    }

    // --- ENQ/DEQ Resource Locking ---

    /** ENQ — acquire a named resource lock */
    public boolean enqueue(String resource) {
        Thread current = Thread.currentThread();
        Thread holder = enqLocks.putIfAbsent(resource.toUpperCase().trim(), current);
        return holder == null || holder == current;
    }

    /** DEQ — release a named resource lock */
    public void dequeue(String resource) {
        enqLocks.remove(resource.toUpperCase().trim(), Thread.currentThread());
    }
