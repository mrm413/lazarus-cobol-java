...
public class Cbact02c extends CobolProgram {
    private int tally = 0;
    ...
    // FILE DESCRIPTORS
    private CobolFile cardfile_file = new CobolFile("CARDFILE", "INDEXED", "SEQUENTIAL");
    ...
    private void para_main() {
        CobolDisplay.display("START OF EXECUTION OF PROGRAM CBACT02C");
        _0000();
        /* RAW: -  */
    }
    ...
    private void cardfile_open() {
        while (!(String.valueOf(end_of_file).equals(String.valueOf("Y")))) {
            if (String.valueOf(end_of_file).equals(String.valueOf("N"))) {
                _1000();
                /* RAW: - CARDFILE-GET-NEXT */
                if (String.valueOf(end_of_file).equals(String.valueOf("N"))) {
                    CobolDisplay.display(String.valueOf(card_record));
                }
            }
        }
        _9000();
    ...
    private void cardfile_close() {
        CobolDisplay.display("END OF EXECUTION OF PROGRAM CBACT02C");
        return;
    }

    private void cardfile_get_next() {
        if (cardfile_file.read(card_record) == FileStatus.AT_END) {
        }
        if (String.valueOf(cardfile_status).equals(String.valueOf("00"))) {
            appl_result.set(String.valueOf(0));
        } else {
            if (String.valueOf(cardfile_status).equals(String.valueOf("10"))) {
        ...
        if (!String.valueOf(appl_aok).trim().isEmpty()) {
            ...
            if (!String.valueOf(appl_eof).trim().isEmpty()) {
                ...
                CobolDisplay.display("ERROR READING CARDFILE");
    ...
    private void appl_result() {
        ...
        if (!String.valueOf(appl_aok).trim().isEmpty()) {
            ...
            CobolDisplay.display("ERROR OPENING CARDFILE");
            io_status.set(String.valueOf(cardfile_status));
            _9910();
            /* RAW: - DISPLAY-IO-STATUS */
            _9999();
    ...
    private void cardfile_close_2() {
        ...
        if (!String.valueOf(appl_aok).trim().isEmpty()) {
            ...
            CobolDisplay.display("ERROR CLOSING CARDFILE");
            io_status.set(String.valueOf(cardfile_status));
            _9910();
            /* RAW: - DISPLAY-IO-STATUS */
            _9999();
