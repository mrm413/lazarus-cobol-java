...
public class Ltcal103 extends CobolProgram {
    private int tally = 0;
    ...
    private CobolString mes_ssrfbn_rate = new CobolString(256); // fallback
    private CobolString ssrfbn_idx = new CobolString(256); // fallback
    private CobolString ssrfbn_tab = new CobolString(256); // fallback
    private CobolString[] wk_ssrfbn_state = new CobolString[100]; // fallback array
    private CobolString mes_pps_state = new CobolString(256); // fallback
    private CobolString[] wk_ssrfbn_reason_all = new CobolString[100]; // fallback array
    ...
    private void h_pr_ipps_pmt_pct() {
        if (Integer.parseInt(String.valueOf(h_ipps_wage_index).trim()) > 1) {
            h_ipps_nat_labor_shr.set(String.valueOf(3593.52));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(1629.62));
        } else {
            h_ipps_nat_labor_shr.set(String.valueOf(3238.35));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(1984.79));
        }
        if (w_ipps_pr_wage_index.compareTo(new BigDecimal(String.valueOf(1).trim())) > 0) {
            h_ipps_pr_labor_shr.set(String.valueOf(1542.72));
            h_ipps_pr_nonlabor_shr.set(String.valueOf(941.52));
        } else {
            h_ipps_pr_labor_shr.set(String.valueOf(1540.23));
            h_ipps_pr_nonlabor_shr.set(String.valueOf(944.01));
        }
        /* RAW: 0100 - */
        return;
    }
    ...
    private void apply_ssrfbn() {
    }

    private void h_ipps_wage_index() {
    }

    private void mes_pps_state() {
        _1950();
        /* RAW: - FIND-SSRFBN THRU 1950 - */
        return;
    }

    private void find_ssrfbn() {
        ssrfbn_idx.set(String.valueOf(1));
        /* SEARCH SSRFBN-TAB */
        {
            boolean _found = false;
    // for (int _idx = 0; _idx < ssrfbn_tab.length && !_found; _idx++) {
    // if (String.valueOf(wk_ssrfbn_state[Integer.parseInt(String.valueOf(ssrfbn_idx).trim()) - 1]).equals(String.valueOf(mes_pps_state))) {
    // /* RAW: MES-SSRFBN  */
    // _found = true;
    // }
    // }
            if (!_found) {
                pps_rtc = new BigDecimal(String.valueOf(68).trim()).intValue();
                _1950();
                return;
            }
        }
    ...
    private void _1950() { /* stub — external/COPY */ }
