...
public class RunReportwriter009SampleChargeReport3 extends CobolProgram {
    ...
    private CobolString tr_item_department = new CobolString(256);
    private CobolString tr_item_cost = new CobolString(256);
    ...
    private void process_transaction_data() {
        ...
        ws_discount_amt = new BigDecimal(String.valueOf((Integer.parseInt(String.valueOf(tr_item_cost).trim()) * Integer.parseInt(String.valueOf(discount[discount_ix - 1]).trim()))).trim()).intValue();
        ws_charge_amt = new BigDecimal(String.valueOf((Integer.parseInt(String.valueOf(tr_item_cost).trim()) - ws_discount_amt)).trim()).intValue();
        /* RAW: GENERATE  */
    }

    private void customer_report_3() {
        if (transaction_data.read(null) == FileStatus.AT_END) {
