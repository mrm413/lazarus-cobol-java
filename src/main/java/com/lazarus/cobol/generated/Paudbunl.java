...
public class Paudbunl extends CobolProgram {
    private int tally = 0;
    ...
    // FILE DESCRIPTORS
    private CobolFile opfile1 = new CobolFile("OUTFIL1", "SEQUENTIAL", "SEQUENTIAL");
    // File status: WS-OUTFL1-STATUS
    private CobolFile opfile2 = new CobolFile("OUTFIL2", "SEQUENTIAL", "SEQUENTIAL");
    // File status: WS-OUTFL2-STATUS
    ...
    private void initialize() {
        ...
        CobolDisplay.display("STARTING PROGRAM PAUDBUNL::");
        CobolDisplay.display("*-------------------------------------*");
        CobolDisplay.display("TODAYS DATE            :" + String.valueOf(current_date));
        CobolDisplay.display(" ");
        opfile1.open("OUTPUT");
        if ((String.valueOf(ws_outfl1_status).equals(String.valueOf(" ")) || true)) {
            /* CONTINUE */
        } else {
            CobolDisplay.display("ERROR IN OPENING OPFILE1:" + String.valueOf(ws_outfl1_status));
            _9999();
            /* RAW: - ABEND */
        }
        opfile2.open("OUTPUT");
        if ((String.valueOf(ws_outfl2_status).equals(String.valueOf(" ")) || true)) {
            /* CONTINUE */
        } else {
            CobolDisplay.display("ERROR IN OPENING OPFILE2:" + String.valueOf(ws_outfl2_status));
            _9999();
            /* RAW: - ABEND */
        }
        /* RAW: 1000 - */
    ...
    private void root_unqual_ssa() {
        if (String.valueOf(paut_pcb_status).equals(String.valueOf(" "))) {
            ...
            if (CobolIntrinsics.isNumeric(pa_acct_id)) {
                opfile1.write(opfil1_rec);
                ws_end_of_child_seg.set("");
                _3000();
    ...
    private void opfile1() {
        if ((String.valueOf(ws_outfl1_status).equals(String.valueOf(" ")) || true)) {
            /* CONTINUE */
        } else {
            CobolDisplay.display("ERROR IN CLOSING 1ST FILE:" + String.valueOf(ws_outfl1_status));
        }
    }

    private void opfile2() {
        if ((String.valueOf(ws_outfl2_status).equals(String.valueOf(" ")) || true)) {
            /* CONTINUE */
        } else {
            CobolDisplay.display("ERROR IN CLOSING 2ND FILE:" + String.valueOf(ws_outfl2_status));
        }
        /* RAW: 4000 - */
        return;
    }
    ...
    private void abend() {
        CobolDisplay.display("IMSUNLOD ABENDING ...");
