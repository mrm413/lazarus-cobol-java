...
public class Ltcal154 extends CobolProgram {
    private int tally = 0;
    ...
    private void pps_cbsa_2() {
        _1900();
        /* RAW: - GET-IPPS-WAGE-INDEX THRU 1900 - */
        return;
    }
    ...
    private void pps_nat_nonlabor_pct() {
        if (String.valueOf(p_new_hosp_qual_ind).equals(String.valueOf("1"))) {
            pps_std_fed_rate = new BigDecimal(String.valueOf(41043.71).trim()).intValue();
        } else {
    ...
    private void h_pr_ipps_pmt_pct() {
        if (Integer.parseInt(String.valueOf(h_ipps_wage_index).trim()) > 1) {
            h_ipps_nat_labor_shr.set(String.valueOf(3784.75));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(1653.1));
        } else {
            h_ipps_nat_labor_shr.set(String.valueOf(3371.47));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(2066.38));
        }
        if (w_ipps_pr_wage_index.compareTo(new BigDecimal(String.valueOf(1).trim())) > 0) {
            h_ipps_pr_labor_shr.set(String.valueOf(1609.97));
            h_ipps_pr_nonlabor_shr.set(String.valueOf(937.45));
        } else {
            h_ipps_pr_labor_shr.set(String.valueOf(1579.4));
            h_ipps_pr_nonlabor_shr.set(String.valueOf(968.02));
        }
    }
    ...
    private void get_ipps_wage_index() {
    }

    private void h_ipps_wage_index() {
        /* SEARCH ALL IBW-ENTRY — binary search */
        {
            boolean _found = false;
            /* Binary search implementation needed */
            if (!_found) {
    ...
    private void subclauseii_final_pay() {
        pps_final_pay_amt = new BigDecimal(String.valueOf((p_new_fac_spec_rate * b_cst_rpt_days)).trim()).intValue();
        if (pps_outlier_pay_amt > 0) {
            pps_rtc = new BigDecimal(String.valueOf(29).trim()).intValue();
        } else {
            pps_rtc = new BigDecimal(String.valueOf(28).trim()).intValue();
        }
    }
