...
public class Comen01c extends CobolProgram {
    private int tally = 0;
    ...
    private CobolString ws_menu_opt_txt = new CobolString(40);
    ...
    private CobolString comen1ao = new CobolString(256); // fallback
    ...
    private CobolString cdemo_menu_opt_count = new CobolString(256); // fallback
    private CobolString cdemo_usrtyp_user = new CobolString(256); // fallback
    private CobolString[] cdemo_menu_opt_usrtype = new CobolString[100]; // fallback array
    ...
    private CobolString[] cdemo_menu_opt_pgmname = new CobolString[100]; // fallback array
    ...
    private CobolString[] cdemo_menu_opt_name = new CobolString[100]; // fallback array
    ...
    private CobolString[] cdemo_menu_opt_num = new CobolString[100]; // fallback array
    ...
    private CobolString optn011o = new CobolString(256); // fallback
    private CobolString optn012o = new CobolString(256); // fallback
    ...
    private void process_enter_key() {
        ...
        if (((!CobolIntrinsics.isNumeric(ws_option) || ws_option > Integer.parseInt(String.valueOf(cdemo_menu_opt_count).trim())) || String.valueOf(ws_option).equals(String.valueOf(0)))) {
            ws_err_flg.set(String.valueOf("Y"));
            ws_message.set(String.valueOf("Please enter a valid option number..."));
            send_menu_screen();
        }
        if ((!String.valueOf(cdemo_usrtyp_user).trim().isEmpty() && String.valueOf(cdemo_menu_opt_usrtype[ws_option - 1]).equals(String.valueOf("A")))) {
            err_flg_on.set(String.valueOf(cb_true));
            ws_message.set(String.valueOf(" "));
            ws_message.set(String.valueOf("No access - Admin Only option... "));
            send_menu_screen();
        }
        if (!(!String.valueOf(err_flg_on).trim().isEmpty())) {
            if (!String.valueOf(cdemo_menu_opt_pgmname[ws_option - 1]).trim().isEmpty()) {
                /* RAW: = 'COPAUS0C' EXEC CICS INQUIRE PROGRAM */
                if (String.valueOf(eibresp).equals(String.valueOf(dfhresp[Integer.parseInt(String.valueOf(normal).trim()) - 1]))) {
                    ...
                    if (program[Integer.parseInt(String.valueOf(cdemo_menu_opt_pgmname[ws_option - 1]).trim()) - 1] == null) program[Integer.parseInt(String.valueOf(cdemo_menu_opt_pgmname[ws_option - 1]).trim()) - 1] = new CobolString(256);
                    program[Integer.parseInt(String.valueOf(cdemo_menu_opt_pgmname[ws_option - 1]).trim()) - 1].set(String.valueOf(0));
                    ...
                    errmsgc.set(String.valueOf(dfhred));
                    {
                        ...
                        _sb.append(CobolString.delimitedBy(String.valueOf(cdemo_menu_opt_name[ws_option - 1]), "  "));
                        _sb.append(" is not installed...");
                        ws_message.set(_sb.toString());
                    }
                }
            } else if (!String.valueOf(CobolString.refMod(cdemo_menu_opt_pgmname[ws_option - 1], 1, 5)).trim().isEmpty()) {
                /* RAW: = 'DUMMY' */
                ws_message.set(String.valueOf(" "));
    ...
    private void send_menu_screen() {
        ...
        if (map[0 /* non-numeric literal: COMEN1A */ - 1] == null) map[0 /* non-numeric literal: COMEN1A */ - 1] = new CobolString(256);
        map[0 /* non-numeric literal: COMEN1A */ - 1].set(String.valueOf(ws_message));
        if (mapset[0 /* non-numeric literal: COMEN01 */ - 1] == null) mapset[0 /* non-numeric literal: COMEN01 */ - 1] = new CobolString(256);
        mapset[0 /* non-numeric literal: COMEN01 */ - 1].set(String.valueOf(ws_message));
        if (from[Integer.parseInt(String.valueOf(comen1ao).trim()) - 1] == null) from[Integer.parseInt(String.valueOf(comen1ao).trim()) - 1] = new CobolString(256);
        from[Integer.parseInt(String.valueOf(comen1ao).trim()) - 1].set(String.valueOf(ws_message));
    ...
    private void receive_menu_screen() {
        /* RAW: EXEC CICS RECEIVE MAP ( 'COMEN1A' */
    }
    ...
    private void build_menu_options() {
        for (ws_idx = 1; !(ws_idx > Integer.parseInt(String.valueOf(cdemo_menu_opt_count).trim())); ws_idx += 1) {
            ws_menu_opt_txt.set(String.valueOf(" "));
            {
                StringBuilder _sb = new StringBuilder();
                _sb.append(String.valueOf(cdemo_menu_opt_num[ws_idx - 1]));
                _sb.append(". ");
                _sb.append(String.valueOf(cdemo_menu_opt_name[ws_idx - 1]));
                ws_menu_opt_txt.set(_sb.toString());
            }
            if (ws_idx == 1) {
                optn001o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 2) {
                optn002o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 3) {
                optn003o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 4) {
                optn004o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 5) {
                optn005o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 6) {
                optn006o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 7) {
                optn007o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 8) {
                optn008o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 9) {
                optn009o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 10) {
                optn010o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 11) {
                optn011o.set(String.valueOf(ws_menu_opt_txt));
            } else if (ws_idx == 12) {
                optn012o.set(String.valueOf(ws_menu_opt_txt));
            } else {
                /* CONTINUE */
            }
        }
