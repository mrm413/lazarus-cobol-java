...
public class RunManualScreen045AcceptFieldWithCursorDataItem extends CobolProgram {
    private int tally = 0;
    ...
    private void testme() {
        ...
        CobolDisplay.display("(third column in field) below, then position it at" + String.valueOf(line) + String.valueOf(2) + String.valueOf(column) + String.valueOf(1));
        CobolDisplay.display("the 'E' (fifth column) and press ENTER." + String.valueOf(line) + String.valueOf(3) + String.valueOf(column) + String.valueOf(1));
        ws_x_20.set(String.valueOf("ABCDEFG   "));
        cur_pos = new BigDecimal(String.valueOf(3).trim()).intValue();
        ws_x_20.set(String.valueOf(CobolDisplay.accept()));
        /* RAW: LINE 5 COLUMN 3 WITH AUTO-SKIP */
        if ((cur_pos == 5 && cob_crt_status == 0)) {
            return;
        } else {
            return;
        }
    ...
    public static void main(String[] args) {
        new RunManualScreen045AcceptFieldWithCursorDataItem().run();
    }
}
