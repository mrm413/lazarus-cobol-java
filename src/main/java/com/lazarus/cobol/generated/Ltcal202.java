...
public class Ltcal202 extends CobolProgram {
    private int tally = 0;
    ...
    private CobolString h_discharge_date = new CobolString(256); // fallback
    private CobolString h_admiss_date_int = new CobolString(256); // fallback
    private CobolString h_admission_date = new CobolString(256); // fallback
    private CobolString h_bdgt_neut_factor = new CobolString(256); // fallback
    private CobolString h_site_neutral_ipps_adj = new CobolString(256); // fallback
    ...
    private CobolString h_sn_ipps_4comparison = new CobolString(256); // fallback
    private CobolString h_outlier_ipps_comparable = new CobolString(256); // fallback
    private CobolString h_sn_cost_4comparison = new CobolString(256); // fallback
    ...
    private CobolString h_ipps_like_amt_outlier = new CobolString(256); // fallback
    private CobolString h_ipps_like_amt = new CobolString(256); // fallback
    ...
    private CobolString h_pre_dpp_pay = new CobolString(256); // fallback
    private CobolString b_ltch_dpp_adjustment = new CobolString(256); // fallback
    ...
    private void pps_nat_nonlabor_pct() {
        if (String.valueOf(p_new_hosp_qual_ind).equals(String.valueOf("1"))) {
            pps_std_fed_rate = new BigDecimal(String.valueOf(42677.64).trim()).intValue();
        } else {
    ...
    private void h_ipps_wage_index() {
        if (Integer.parseInt(String.valueOf(h_ipps_wage_index).trim()) > 1) {
            h_ipps_nat_labor_shr.set(String.valueOf(3959.1));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(1837.53));
        } else {
            h_ipps_nat_labor_shr.set(String.valueOf(3593.91));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(2202.72));
        }
    }
    ...
    // _3100();
        ...
        if (((((((b_review_code == 1 && !String.valueOf(psych_rehab_drg).trim().isEmpty()) || b_review_code == 2) || b_review_code == 3) || b_review_code == 6) || b_review_code == 7) || b_review_code == 8)) {
            h_discharge_date.set(String.valueOf(b_discharge_date));
    // h_admiss_date_int.set(String.valueOf((CobolIntrinsics.integer_of_date(h_discharge_date) - b_los)));
    // h_admission_date.set(String.valueOf(CobolIntrinsics.date_of_integer(h_admiss_date_int)));
            if (Integer.parseInt(String.valueOf(h_admission_date).trim()) > 20200126) {
                pmt_standard_new.set(String.valueOf(cb_true));
    // _3100();
                return;
            } else {
                h_bdgt_neut_factor.set(String.valueOf(0.949));
                h_site_neutral_ipps_adj.set(String.valueOf(0.954));
                if (pps_blend_year == 8) {
    ...
    private void pps_site_neutral_ipps_pmt() {
    // _6000();
        /* RAW: - CALC-HIGH-COST-OUTLIER THRU 6000 - */
        return;
    }

    private void h_outlier_ipps_comparable() {
        pps_outlier_pay_amt = 0;
        ws_secondary_pmt_type_snt.set("");
    }

    private void pps_site_neutral_ipps_pmt_2() {
    }

    private void h_sn_cost_4comparison() {
        h_sn_ipps_4comparison.set(String.valueOf((Integer.parseInt(String.valueOf(h_site_neutral_ipps_adj).trim()) * ((Integer.parseInt(String.valueOf(h_ipps_per_diem).trim()) + Integer.parseInt(String.valueOf(h_outlier_ipps_comparable).trim()))))));
        if (String.valueOf(h_sn_cost_4comparison).compareTo(String.valueOf(h_sn_ipps_4comparison)) < 0) {
    ...
    private void calc_ipps_like_amt() {
    // _3650();
        /* RAW: - SS-IPPS-COMP-PMT THRU 3650 - */
    }

    private void ss_ipps_comp_pmt_exit_2() {
        pps_outlier_threshold = new BigDecimal(String.valueOf((Integer.parseInt(String.valueOf(h_ipps_pay_amt).trim()) + Integer.parseInt(String.valueOf(h_fixed_loss_amt_snt).trim()))).trim()).intValue();
        if (pps_fac_costs > pps_outlier_threshold) {
            h_ipps_like_amt_outlier.set(String.valueOf((((pps_fac_costs - pps_outlier_threshold)) * 0.8)));
        }
        h_ipps_like_amt.set(String.valueOf((Integer.parseInt(String.valueOf(h_ipps_pay_amt).trim()) + Integer.parseInt(String.valueOf(h_ipps_like_amt_outlier).trim()))));
        /* RAW: 3500 - */
        return;
    }
    ...
    private void ss_ipps_comp_pmt_exit_4() {
        return;
    }
    ...
    private void calc_high_cost_outlier() {
        ...
        if (!String.valueOf(pmt_blend).trim().isEmpty()) {
            ...
            pps_outlier_pay_amt = new BigDecimal(String.valueOf((Integer.parseInt(String.valueOf(h_outlier_pay_amt_std).trim()) + ((Integer.parseInt(String.valueOf(h_outlier_pay_amt_snt).trim()) * Integer.parseInt(String.valueOf(h_site_neutral_ipps_adj).trim()))))).trim()).intValue();
        }
    ...
    // pps_site_neutral_ipps_pmt = new BigDecimal(String.valueOf(((pps_site_neutral_ipps_pmt * Integer.parseInt(String.valueOf(h_bdgt_neut_factor).trim())) * Integer.parseInt(String.valueOf(h_site_neutral_ipps_adj).trim()))).trim()).intValue();
    // pps_outlier_pay_amt = new BigDecimal(String.valueOf((pps_outlier_pay_amt * Integer.parseInt(String.valueOf(h_site_neutral_ipps_adj).trim()))).trim()).intValue();
    // }
    // if (!String.valueOf(pmt_blend).trim().isEmpty()) {
    ...
    // pps_site_neutral_ipps_pmt = new BigDecimal(String.valueOf((((pps_site_neutral_ipps_pmt * Integer.parseInt(String.valueOf(h_bdgt_neut_factor).trim())) * Integer.parseInt(String.valueOf(h_site_neutral_ipps_adj).trim())) * Integer.parseInt(String.valueOf(h_blend_snt).trim()))).trim()).intValue();
    ...
    // h_pre_dpp_pay.set(String.valueOf(pps_final_pay_amt));
    // if (!String.valueOf(b_ltch_dpp_adjustment).trim().isEmpty()) {
    // _3500();
    // /* RAW: - CALC-IPPS-LIKE-AMT THRU 3500 - */
    // return;
    // }
    // }
    // /* RAW: 8000 - */
