...
public class Ltcal042 extends CobolProgram {
    private int tally = 0;
    ...
    private void assemble_pps_variables() {
        if ((Integer.parseInt(String.valueOf(p_new_fy_begin_date).trim()) >= 20031001 && String.valueOf(b_discharge_date).compareTo(String.valueOf(p_new_fy_begin_date)) >= 0)) {
    ...
    private void move_results() {
        if (pps_rtc < 50) {
            ...
            pps_calc_vers_cd.set(String.valueOf("V04.2"));
        } else {
            pps_data.set("");
            pps_other_data.set("");
