...
public class Ltcal075 extends CobolProgram {
    private int tally = 0;
    ...
    private void h_pr_ipps_pmt_pct() {
        if (w_ipps_wage_index.compareTo(new BigDecimal(String.valueOf(1).trim())) > 0) {
            h_ipps_nat_labor_shr.set(String.valueOf(3397.52));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(1476.97));
        } else {
            h_ipps_nat_labor_shr.set(String.valueOf(3022.18));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(1852.31));
        }
        if (w_ipps_pr_wage_index.compareTo(new BigDecimal(String.valueOf(1).trim())) > 0) {
            h_ipps_pr_labor_shr.set(String.valueOf(1436.12));
            h_ipps_pr_nonlabor_shr.set(String.valueOf(880.2));
        } else {
            h_ipps_pr_labor_shr.set(String.valueOf(1359.68));
            h_ipps_pr_nonlabor_shr.set(String.valueOf(956.64));
        }
        /* RAW: 0100 - */
        return;
    }
    ...
    private void assemble_pps_variables() {
        if (!String.valueOf(p_new_fy_begin_date).trim().isEmpty()) {
            /* RAW: >= FED-FY-BEGIN-07 */
            if (String.valueOf(b_discharge_date).compareTo(String.valueOf(p_new_fy_begin_date)) >= 0) {
            ...
            /* RAW: < FED-FY-BEGIN-06 AND P-NEW-FY-BEGIN-DATE >= FED-FY-BEGIN-05 */
            if ((CobolIntrinsics.isNumeric(w_wage_index1) && w_wage_index1.compareTo(new BigDecimal(String.valueOf(0).trim())) > 0)) {
                pps_wage_index = new BigDecimal(String.valueOf(w_wage_index1).trim()).intValue();
            } else {
                pps_rtc = new BigDecimal(String.valueOf(52).trim()).intValue();
    ...
    private void ss_ipps_comp_pmt() {
        h_oper_ime_teach.set(String.valueOf((1.32 * ((Math.pow(((1 + Integer.parseInt(String.valueOf(h_intern_ratio).trim()))), 0.405) - 1)))));
    ...
    // h_stand_amt_oper_pmt.set(String.valueOf((((((new BigDecimal(String.valueOf(h_ipps_nat_labor_shr).trim()).multiply(w_ipps_wage_index)) + ((Integer.parseInt(String.valueOf(h_ipps_nat_nonlabor_shr).trim()) * Integer.parseInt(String.valueOf(h_oper_cola).trim()))))) * Integer.parseInt(String.valueOf(h_ipps_drg_wgt).trim())) * (((1 + Integer.parseInt(String.valueOf(h_oper_ime_teach).trim())) + Integer.parseInt(String.valueOf(h_oper_dsh).trim()))))));
        ...
        if (!String.valueOf(large_urban).trim().isEmpty()) {
            h_lrgurb_add_on.set(String.valueOf(1.03));
        } else {
    ...
    private void calc_outlier() {
        ...
        if ((((pps_rtc == 1 || true) || true) || String.valueOf(pps_cot_ind).equals(String.valueOf("Y")))) {
    ...
    private void blend() {
        ...
        if ((((pps_rtc == 20 || true) || true) && Integer.parseInt(String.valueOf(h_blend_rtc).trim()) > 0)) {
            pps_rtc = new BigDecimal(String.valueOf((Integer.parseInt(String.valueOf(h_blend_rtc).trim()) + 2)).trim()).intValue();
        } else {
            if (((pps_rtc == 24 || true) && Integer.parseInt(String.valueOf(h_blend_rtc).trim()) > 0)) {
