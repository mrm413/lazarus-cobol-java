...
public class RunFile080ExtfhLineSequentialFiles2 extends CobolProgram {
    private int tally = 0;
    ...
    private static final String program_name = "SEQEXTFH";
    ...
    private CobolString extfh_opcode_isopenoutput = new CobolString(256); // fallback
    private CobolString cb_true = new CobolString(256); // fallback
    private CobolString fcd_parameter_block = new CobolString(256); // fallback
    ...
    private CobolString lseq_filename = new CobolString(256); // fallback
    private CobolString fcd_name_length = new CobolString(256); // fallback
    private CobolString fcd_exclusive_bit = new CobolString(256); // fallback
    private CobolString fcd_lock_mode = new CobolString(256); // fallback
    private CobolString fcd_format_cobol2 = new CobolString(256); // fallback
    private CobolString fcd_file_format = new CobolString(256); // fallback
    private CobolString lseq_recordarea = new CobolString(256); // fallback
    private CobolString fcd_max_rec_length = new CobolString(256); // fallback
    private CobolString fcd_recmode_variable = new CobolString(256); // fallback
    ...
    private CobolString lseq_tx = new CobolString(256); // fallback
    private CobolString lseq_tcount = new CobolString(256); // fallback
    private CobolString extfh_opcode_iswrite = new CobolString(256); // fallback
    private CobolString[] lseq_test_len = new CobolString[100]; // fallback array
    private CobolString fcd_current_rec_len = new CobolString(256); // fallback
    private CobolString fcd_line_count = new CobolString(256); // fallback
    private CobolString fcd_record_address = new CobolString(256); // fallback
    private CobolString extfh_opcode_isclose = new CobolString(256); // fallback
    private CobolString extfh_opcode_isopeninput = new CobolString(256); // fallback
    private CobolString extfh_opcode_isreadsequential = new CobolString(256); // fallback
    ...
    private void para_main() {
        ...
        CobolDisplay.display(String.valueOf(program_name) + ": CREATING LINE SEQUENTIAL FILE.");
        ...
        fcd_name_length.set(String.valueOf(CobolIntrinsics.length(lseq_filename)));
        fcd_lock_mode.set(String.valueOf(fcd_exclusive_bit));
        fcd_file_format.set(String.valueOf(fcd_format_cobol2));
        fcd_max_rec_length.set(String.valueOf(CobolIntrinsics.length(lseq_recordarea)));
        fcd_recording_mode.set(String.valueOf(fcd_recmode_variable));
        ...
        if (String.valueOf(fcd_file_status).equals(String.valueOf("00"))) {
            CobolDisplay.display(String.valueOf(program_name) + ": LINE SEQUENTIAL FILE IS OPEN OUTPUT.");
            for (int _v = 1; !((String.valueOf(lseq_tx).equals(String.valueOf(lseq_tcount)) || !String.valueOf(fcd_file_status).equals(String.valueOf("00")))); _v += 1) {
                lseq_tx.set(String.valueOf(_v));
            }
            extfh_opcode_iswrite.set(String.valueOf(cb_true));
            fcd_current_rec_len.set(String.valueOf(lseq_test_len[Integer.parseInt(String.valueOf(lseq_tx).trim()) - 1]));
            fcd_line_count.set(String.valueOf(1));
            fcd_record_address.set(String.valueOf(address[Integer.parseInt(String.valueOf(lseq_tx).trim()) - 1]));
            call_extfh();
            fcd_check_stat();
            /* RAW: END-PERFORM  */
            if (String.valueOf(fcd_file_status).equals(String.valueOf("00"))) {
                CobolDisplay.display(String.valueOf(program_name) + ": LINE SEQUENTIAL WRITES COMPLETE.");
            } else {
                CobolDisplay.display(String.valueOf(program_name) + ": UNABLE TO CREATE LINE SEQUENTIAL FILE.");
            }
            extfh_opcode_isclose.set(String.valueOf(cb_true));
            call_extfh();
            fcd_check_stat();
            if (String.valueOf(fcd_file_status).equals(String.valueOf("00"))) {
                CobolDisplay.display(String.valueOf(program_name) + ": LINE SEQUENTIAL FILE IS CLOSED.");
            } else {
                CobolDisplay.display(String.valueOf(program_name) + ": UNABLE TO CLOSE LINE SEQUENTIAL FILE.");
            }
        } else {
            CobolDisplay.display(String.valueOf(program_name) + ": UNABLE TO OPEN LINE SEQUENTIAL FILE AS OUTPUT.");
            System.exit(0);
        }
        CobolDisplay.display(String.valueOf(program_name) + ": READING LINE SEQUENTIAL FILE.");
        ...
        if (String.valueOf(fcd_file_status).equals(String.valueOf("00"))) {
            ...
            CobolDisplay.display(String.valueOf(program_name) + ": LINE SEQUENTIAL FILE IS OPEN INPUT.");
            ...
            CobolDisplay.display(String.valueOf(program_name) + ": UNABLE TO OPEN LINE SEQUENTIAL FILE AS INPUT.");
            System.exit(0);
        }
    ...
    private void call_extfh() {
        CobolProgram.call("EXTFH", fcdopcode, fcd_parameter_block);
    }
    ...
    public static void main(String[] args) {
        new RunFile080ExtfhLineSequentialFiles2().run();
    }
}
