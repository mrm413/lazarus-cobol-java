...
public class Ltcal130 extends CobolProgram {
    private int tally = 0;
    ...
    private void pps_nat_nonlabor_pct() {
        if (Integer.parseInt(String.valueOf(b_discharge_date).trim()) >= 20121229) {
            pps_std_fed_rate = new BigDecimal(String.valueOf(40397.96).trim()).intValue();
        } else {
    ...
    private void h_pr_ipps_pmt_pct() {
        if (Integer.parseInt(String.valueOf(h_ipps_wage_index).trim()) > 1) {
            h_ipps_nat_labor_shr.set(String.valueOf(3679.95));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(1668.81));
        } else {
            h_ipps_nat_labor_shr.set(String.valueOf(3316.23));
            h_ipps_nat_nonlabor_shr.set(String.valueOf(2032.53));
        }
        if (w_ipps_pr_wage_index.compareTo(new BigDecimal(String.valueOf(1).trim())) > 0) {
            h_ipps_pr_labor_shr.set(String.valueOf(1564.17));
            h_ipps_pr_nonlabor_shr.set(String.valueOf(954.62));
        } else {
            h_ipps_pr_labor_shr.set(String.valueOf(1561.65));
            h_ipps_pr_nonlabor_shr.set(String.valueOf(957.14));
        }
        /* RAW: 0100 - */
        return;
    }
