...
public class Cousr03c extends CobolProgram {
    private int tally = 0;
    ...
    private CobolString cousr3ao = new CobolString(256); // fallback
    private CobolString usridinl = new CobolString(256); // fallback
    private CobolString cdemo_cu03_usr_selected = new CobolString(256); // fallback
    ...
    private void main_para() {
        ...
        if (String.valueOf(eibcalen).equals(String.valueOf(0))) {
            ...
            if (!(!String.valueOf(cdemo_pgm_reenter).trim().isEmpty())) {
                ...
                cousr3ao.set(String.valueOf("\u0000"));
                usridinl.set(String.valueOf(-1));
                if ((!String.valueOf(cdemo_cu03_usr_selected).equals(String.valueOf(" ")) && true)) {
                    usridini.set(String.valueOf(cdemo_cu03_usr_selected));
                    process_enter_key();
                }
                send_usrdel_screen();
            } else {
    ...
    private void send_usrdel_screen() {
        ...
        if (map[0 /* non-numeric literal: COUSR3A */ - 1] == null) map[0 /* non-numeric literal: COUSR3A */ - 1] = new CobolString(256);
        map[0 /* non-numeric literal: COUSR3A */ - 1].set(String.valueOf(ws_message));
        if (mapset[0 /* non-numeric literal: COUSR03 */ - 1] == null) mapset[0 /* non-numeric literal: COUSR03 */ - 1] = new CobolString(256);
        mapset[0 /* non-numeric literal: COUSR03 */ - 1].set(String.valueOf(ws_message));
        if (from[Integer.parseInt(String.valueOf(cousr3ao).trim()) - 1] == null) from[Integer.parseInt(String.valueOf(cousr3ao).trim()) - 1] = new CobolString(256);
        from[Integer.parseInt(String.valueOf(cousr3ao).trim()) - 1].set(String.valueOf(ws_message));
    ...
    private void receive_usrdel_screen() {
        /* RAW: EXEC CICS RECEIVE MAP ( 'COUSR3A' */
    }
    ...
    private void end_exec_5() {
        if (String.valueOf(ws_resp_cd).equals(String.valueOf(dfhresp[Integer.parseInt(String.valueOf(normal).trim()) - 1]))) {
            /* CONTINUE */
            ws_message.set(String.valueOf("Press PF5 key to delete this user ..."));
    ...
    private void delete_user_sec_file() {
        /* RAW: EXEC CICS */
        dataset.delete();
        /* RAW: ( WS-USRSEC-FILE ) RESP ( WS-RESP-CD */
    }
